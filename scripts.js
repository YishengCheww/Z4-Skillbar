$(document).ready(function(){
  ...
  document.onkeydown = function (data) {
      ...
      if (data.which == 69) {
        closeMain()
        $.post('http://reload-skillbar/taskEnd', JSON.stringify({taskResult: percent}));
      }
  }
  ...
  window.addEventListener('message', function(event){
    ...
    if(item.runUpdate === true) {
      ...
      if (item.Length < (item.chance + item.skillGap) && item.Length > (item.chance)) {
        $('.skillprogress').css("background-color","rgba(105,230,158,0.9)");

      } else {
        $('.skillprogress').css("background-color","rgba(120,113,113,0.4)");
      }
    ...
    if(item.closeFail === true) {
      closeMain()
      $.post('http://reload-skillbar/taskCancel', JSON.stringify({tasknum: curTask}));
    }
